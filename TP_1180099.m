 ...
 %calculo do angulos dos angulos correspondenes à pos inicial, intermédia e final
    ...
    for k=1:2  
      ...
      t(k,:)=t(k,:)+t(k-1,length(t));    
        ...
        figure(10);
        hold on
        plot(t(k,:),rad2deg(theta1_int(k,:)), 'DisplayName','posição junta 1')
        plot(t(k,:),rad2deg(theta2_int(k,:)), 'DisplayName','posição junta 2')
        plot(t(k,:),rad2deg(theta3_int(k,:)), 'DisplayName','posição junta 3')
        title('Posição theta')
        ...
        plot(t(k,:),theta1pp_int(k,:)*180/pi,'DisplayName','aceleração junta 1')
        plot(t(k,:),theta2pp_int(k,:)*180/pi,'DisplayName','aceleração junta 2')
        plot(t(k,:),theta3pp_int(k,:)*180/pi,'DisplayName','aceleração junta 3')
        xlabel('{\it t} (s)','FontSize',12);
