...
prompt = {'Introduza h (m)','Introduza L1 (m)','Introduza L2 (m)','Introduza x1 inicial (m)',...
 ...
 for j=1:2
    ...
    for j=1:2   
       ...
       %aceleracao angular das 3 juntas 
        ...
        subplot(3,3,2)
        hold on
        title('Posição da extremidade final do elo L1 no espaço operacional')
        ...
        subplot(3,3,5)
        hold on
        title('Velocidade da extremidade final do elo L1 no espaço operacional')
        ...
        subplot(3,3,8)
        hold on
        title('Aceleração na extremidade final do elo L1 no espaço operacional')
        plot(t(j,3:length(t)),ax1(j,3:length(ax1)),'c.','DisplayName',' x ');
        plot(t(j,3:length(t)),ay1(j,3:length(ay1)),'m.','DisplayName','aceleração y no manipulador');
        plot(t(j,3:length(t)),az1(j,3:length(ay1)),'k.','DisplayName','aceleração z no manipulador');
        if j==2 line=vline(tf(1),'k--'); end %desenha a linha 
        xlabel('{\it t} (s)','FontSize',12);
